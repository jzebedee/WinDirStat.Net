...
public abstract class ViewModelRelayCommand : ObservableRecipient
{
    ...
    private readonly ConcurrentDictionary<string, ICommand> commands = new();
    ...
    protected TCommand GetCommand<TCommand>(TCommand newCommand, [CallerMemberName] string commandName = null)
        ...
        => (TCommand)commands.GetOrAdd(commandName, newCommand);
    ...
    /// <param name="commandName">The name of the command.</param>
    /// <returns>The command from the command list.</returns>
    protected TCommand GetCommand<TCommand>(Func<TCommand> createCommand, [CallerMemberName] string commandName = null)
        where TCommand : ICommand
        => (TCommand)commands.GetOrAdd(commandName, _ => createCommand());
    ...
    protected IRelayCommand GetCommand(Action execute, [CallerMemberName] string commandName = null)
        => GetCommand(execute, null, commandName);
    ...
    protected IRelayCommand GetCommand(Action execute, Func<bool> canExecute, [CallerMemberName] string commandName = null)
        => GetCommand(() => relayFactory.Create(execute, canExecute), commandName);
