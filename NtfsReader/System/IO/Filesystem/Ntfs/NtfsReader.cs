...
public sealed partial class NtfsReader : IDisposable
{
    ...
    /// </summary>
    private sealed class FragmentWrapper : INtfsFragment
    {
        readonly StreamWrapper _owner;
        Fragment _fragment;
    ...
    /// Add some functionality to the basic stream
    /// </summary>
    private sealed class StreamWrapper : INtfsStream
    {
        NtfsReader _reader;
        ...
        public IList<INtfsFragment> Fragments
        {
            get
            {
                ...
                List<INtfsFragment> newFragments = new();
    ...
    /// </summary>
    private sealed class NodeWrapper : INtfsNode
    {
        NtfsReader _reader;
        uint _nodeIndex;
        Node _node;
        ...
        public FileAttributes Attributes => _node.Attributes;
        ...
        public IList<INtfsStream> Streams
        {
            get
            {
                ...
                List<INtfsStream> newStreams = new();
        ...
        public DateTime CreationTime
        {
            get
            {
                ...
                return DateTime.FromFileTimeUtc((long)_reader._standardInformations[_nodeIndex].CreationTime);
            }
        }
        ...
        public DateTime LastChangeTime
        {
            get
            {
                ...
                return DateTime.FromFileTimeUtc((long)_reader._standardInformations[_nodeIndex].LastChangeTime);
            }
        }
        ...
        public DateTime LastAccessTime
        {
            get
            {
                ...
                return DateTime.FromFileTimeUtc((long)_reader._standardInformations[_nodeIndex].LastAccessTime);
            }
        }
    ...
    /// </summary>
    private sealed class DiskInfoWrapper : INtfsDiskInfo
    {
        public ushort BytesPerSector;
        public byte SectorsPerCluster;
        public ulong TotalSectors;
        ...
        ushort INtfsDiskInfo.BytesPerSector => BytesPerSector;

        byte INtfsDiskInfo.SectorsPerCluster => SectorsPerCluster;

        ulong INtfsDiskInfo.TotalSectors => TotalSectors;

        ulong INtfsDiskInfo.MftStartLcn => MftStartLcn;

        ulong INtfsDiskInfo.Mft2StartLcn => Mft2StartLcn;

        uint INtfsDiskInfo.ClustersPerMftRecord => ClustersPerMftRecord;

        uint INtfsDiskInfo.ClustersPerIndexRecord => ClustersPerIndexRecord;

        ulong INtfsDiskInfo.BytesPerMftRecord => BytesPerMftRecord;

        ulong INtfsDiskInfo.BytesPerCluster => BytesPerCluster;

        ulong INtfsDiskInfo.TotalClusters => TotalClusters;
    ...
    Dictionary<string, int> _nameIndex = new(128 * 1024, StringComparer.Ordinal);
    ...
    {
        NativeOverlapped overlapped = new(absolutePosition);
            ...
            if (attribute->Nonresident == 0)
            {
                ...
                switch (attribute->AttributeType)
                {
                    ...
                    case AttributeType.AttributeStandardInformation:
                        ...
                        node.Attributes |= (FileAttributes)attributeStandardInformation->FileAttributes;
    ...
    private unsafe bool ProcessMftRecord(byte* buffer, ulong length, uint nodeIndex, out Node node, List<Stream> streams, bool isMftNode)
    {
        ...
        {
            node.Attributes |= FileAttributes.Directory;
        }
